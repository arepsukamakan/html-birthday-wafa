...
body::before{content:"\00A9  arep.dnish";color:white;opacity:.5;font-size:10px;position:fixed;bottom:25px;right:25px;z-index:2}
...
#pergeseran p{background:rgba(0, 0, 0, .2);border:1px dashed #fff;border-radius:14px;padding:15px;display:flex;flex-wrap:nowrap;text-align:center;line-height:1.4em;align-items:center;justify-content:center;flex-shrink:0; width:90%;height:150px;margin:0 15px 0 0; scroll-snap-align:center}
...
#keterangan{position:relative;bottom:-60px;color:white;font-size:13px;opacity:0;transition:all .7s ease;}
...
.dots{position:relative;bottom:-30px;left:50%;transform:translateX(-50%);display:flex;justify-content:center;align-items:center;width:100%;z-index:2;opacity:0;transition:all .8s ease;}
.dot{height:6px;width:6px;border-radius:50%;margin:0 5px;background-color:#aaa;cursor:pointer;opacity:.4;}
.dot.active{background-color:#fff;opacity:1;}

/* GIFTS */
#teksgift{color:white;font-family:var(--gaya-font2);font-size:20px;font-weight:700;text-shadow: 0px 2px 2px rgba(0, 0, 0, .8);top:55%;left:50%;transform:translate(-50%,-55%) scale(0)}
...
#gift{width:150px;height:150px;top:30%;left:50%;transform:translate(-50%,-30%) scale(0)}
